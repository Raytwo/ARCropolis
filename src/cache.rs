...
fn cache_entry_parser<R: Read + Seek>(
    reader: &mut R,
    ro: &ReadOptions,
    _: (),
) -> BinResult<HashMap<HashToIndex, (HashToIndex, u32)>> {
    let mut options = *ro;
    let count = match options.count.take() {
        Some(x) => x,
        None => {
            panic!("Missing count for HashMap");
        }
    };
    ...
    for _ in 0..count {
        map.insert(reader.read_le().unwrap(), reader.read_le().unwrap());
    }
...
impl UnshareCache {
    ...
    pub fn write(
        arc: &LoadedArc,
        cache_map: &HashMap<HashToIndex, (HashToIndex, u32)>,
        path: &PathBuf,
    ) -> std::io::Result<()> {
        let file = std::fs::File::create(path)?;
        let mut writer = std::io::BufWriter::new(file);
        let out_vec: Vec<(HashToIndex, (HashToIndex, u32))> =
            cache_map.iter().map(|(hash, dir)| (*hash, *dir)).collect();
        let entries = out_vec.len() as u32;
        unsafe {
            writer.write_all(b"ARC_VER\0").unwrap();
            writer
                .write_all(std::slice::from_raw_parts(
                    &(*arc.fs_header).version as *const u32 as *const u8,
                    std::mem::size_of::<u32>(),
                ))
                .unwrap();
            writer.write_all(b"MOD_VER\0").unwrap();
            writer
                .write_all(std::slice::from_raw_parts(
                    &0u32 as *const u32 as *const u8,
                    std::mem::size_of::<u32>(),
                ))
                .unwrap();
            writer
                .write_all(std::slice::from_raw_parts(
                    &entries as *const u32 as *const u8,
                    std::mem::size_of::<u32>(),
                ))
                .unwrap();
            for (hash, dir_info) in out_vec.iter() {
                writer
                    .write_all(std::slice::from_raw_parts(
                        hash as *const HashToIndex as *const u8,
                        std::mem::size_of::<HashToIndex>(),
                    ))
                    .unwrap();
                writer
                    .write_all(std::slice::from_raw_parts(
                        &dir_info.0 as *const HashToIndex as *const u8,
                        std::mem::size_of::<HashToIndex>(),
                    ))
                    .unwrap();
                writer
                    .write_all(std::slice::from_raw_parts(
                        &dir_info.1 as *const u32 as *const u8,
                        std::mem::size_of::<u32>(),
                    ))
                    .unwrap();
            }
