...
use super::{AdditionContext, FromPathExt, SearchContext, SearchEx, lookup};
...
pub fn add_shared_file(ctx: &mut AdditionContext, path: &Path, shared_to: Hash40) {
    let info_indice_idx = match ctx.get_file_info_from_hash(shared_to) {
        ...
        Err(e) => {
            error!("Failed to find file '{}' ({:#x}) when attempting to share file to it.", hashes::find(shared_to), shared_to.0);
            return;
        }
    };
    ...
    lookup::add_shared_file(path.smash_hash().unwrap() /* we can unwrap because of FilePath::from_path being successful */, shared_to);
}
